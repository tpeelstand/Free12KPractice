...
// OPTIMIZATION: Clear transient caches when Math content or taxonomy changes
add_action('save_post_math_skill', function($post_id) {
    // Clear all Math transients
    delete_transient('math_all_domains');
    // Clear child grade caches for all grades
    $all_grades = get_terms(array('taxonomy' => 'math_grade', 'fields' => 'ids', 'hide_empty' => false));
    if (!is_wp_error($all_grades)) {
        foreach ($all_grades as $grade_id) {
            delete_transient('math_grade_children_' . $grade_id);
        }
    }
});

add_action('edit_math_grade', function($term_id) {
    delete_transient('math_grade_children_' . $term_id);
    delete_transient('math_all_domains');
});

add_action('delete_math_grade', function($term_id) {
    delete_transient('math_grade_children_' . $term_id);
    delete_transient('math_all_domains');
});
...
function get_domains_with_grades_callback() {
    ...
    $cache_key = 'math_grade_children_' . $parent_id;
    ...
    $domains_cache_key = 'math_all_domains';
    ...
    $output_cache_key = 'math_domains_output_' . $parent_id;
        ...
        foreach ($post_id_chunks as $chunk) {
            $chunk_results = $wpdb->get_results(
                "SELECT tr.object_id, t.term_id FROM {$wpdb->term_relationships} tr
                 ...
                 WHERE tt.taxonomy = 'math_grade' AND tr.object_id IN (" . implode(',', $chunk) . ")"
            );
            if ($chunk_results) {
        ...
        foreach ($post_id_chunks as $chunk) {
            $chunk_results = $wpdb->get_results(
                "SELECT tr.object_id, t.term_id FROM {$wpdb->term_relationships} tr
                 INNER JOIN {$wpdb->term_taxonomy} tt ON tr.term_taxonomy_id = tt.term_taxonomy_id
                 INNER JOIN {$wpdb->terms} t ON tt.term_id = t.term_id
                 WHERE tt.taxonomy = 'math_domain' AND tr.object_id IN (" . implode(',', $chunk) . ")"
            );
            if ($chunk_results) {
        ...
        foreach ($post_id_chunks as $chunk) {
            $chunk_results = $wpdb->get_results(
                "SELECT tr.object_id, t.name FROM {$wpdb->term_relationships} tr
                 INNER JOIN {$wpdb->term_taxonomy} tt ON tr.term_taxonomy_id = tt.term_taxonomy_id
                 INNER JOIN {$wpdb->terms} t ON tt.term_id = t.term_id
                 WHERE tt.taxonomy = 'csv_source' AND tr.object_id IN (" . implode(',', $chunk) . ")"
            );
            if ($chunk_results) {
    ...
    $output = ob_get_clean();
    if ($output) {
        set_transient('math_domains_output_' . $parent_id, $output, 24 * HOUR_IN_SECONDS);
    }
